...
称作**中断向量**的位置（靠近内存底部的固定位置），个人感觉应该是内核所占内存的底部吧。它包含中断
服务程序的入口地址，也就说中断向量其实就是一个指针吧。假设当一个磁盘中断发生时，用户进程3正在运行，
...
机制存入堆栈的那部分信息，并将堆栈指针指向一个由进程处理程序所使用的临时堆栈。话说这里的进程处理程序是什么鬼，
还有这里的意思就是，中断例程会将之前硬件保存在堆栈中的寄存器的值保存到进程表中对应的表项中。这里考虑一下，
如果我们不使用硬件保存寄存器的值，那么如果直接跳转到中断向量的话，会导致当前寄存器的值消失，但是话说，
如果光由硬件处理的话，它是如何找到堆栈的位置，不对，寄存器中不是有堆栈指针么，机智。     
...
陷入内核，不需要上下文切换，不需要对内存高速缓存进行刷新（那说明这些操作是进程切换需要的）。   

第二个优点是我们可以定制进程中线程的调度算法。      
...
虽然解决了阻塞调用和页面中断时的问题，但是内核线程也有其自身的问题，例如说，使用fork调用创建一个进程时，
新进程是只有一个线程还是有与旧进程同样多的线程（看这个意思，用户级线程由于是我们自己实现线程管理的，所以
可能在创建进程时是可以控制线程的创建与否的）。   

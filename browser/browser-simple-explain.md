...
一旦所有检查都完事，网络线程就通知 UI 线程一切准备就绪。UI 线程就会找一个渲染线程会负责页面的渲染。    
...
SW 是运行在一个渲染线程中的，那当发出导航请求的时候，browser 进程是如何知道这个站点存在一个 SW 呢？   


当注册一个 SW 时，SW 的 scope 会被保存下来。当导航动作发生时，网络线程会检查域中注册的 SW 的
scopes，如果一个 SW 为这个 URL 注册了，那 UI 线程就搜寻一个渲染线程区执行 SW 的代码。   
...
渲染线程负责了一个 tab 中的所有东西。主线程处理大部分要发送给用户的代码。Compositor 和 Raster
线程也运行在渲染线程内。    
